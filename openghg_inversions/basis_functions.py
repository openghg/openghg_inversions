...
Create basis functions for domain
...
#     load a Fields file to get the domain
    ...
    basis_ds = xr.Dataset({'basis':(['lat','lon','time'], basis_grid)}, \
            coords = {'lon':(['lon'], lon), \
                      'lat':(['lat'], lat), \
                      'time':(['time'],[time]) ,\
...
def basis_bc_blocks(domain, time, basis_case = "NESW", vertical=1):
    ...
    basis_ds = xr.Dataset({'bc_basis_n':(['height','lon','region','time'], basis_grid_n), \
...
def basis_bc_all_gradients(domain, time, species, units='ppb', basis_case='horiz-strat-grad'):
    ...
    bc_ds =  utils.boundary_conditions(domain, species)
