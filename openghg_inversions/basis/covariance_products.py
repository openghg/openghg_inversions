...
def _preflight_native_covariance_projection(
    *,
    covariance: InvertibleNativeCovarianceAction,
    basis_prolongation: xr.DataArray,
    state_dim: str,
    ...
    observation_dim: str,
    observation_covariance: Literal["dense", "diagonal"],
    observation_batch_size: int,
) -> tuple[tuple[str, ...], int]:
    """Validate projection structure without materializing sensitivity data."""
    ...
    if not all(isinstance(dim, str) for dim in native_dims):
    ...
    expected_sensitivity_dims = {observation_dim, *native_dims}
    if (
        set(native_sensitivity.dims) != expected_sensitivity_dims
        or len(native_sensitivity.dims) != len(expected_sensitivity_dims)
    ):
        raise ValueError(
            "native_sensitivity must have exactly observation and native dimensions; "
            f"got {native_sensitivity.dims!r}"
        )
    expected_prolongation_dims = {state_dim, *native_dims}
    if (
        set(basis_prolongation.dims) != expected_prolongation_dims
        or len(basis_prolongation.dims) != len(expected_prolongation_dims)
    ):
        raise ValueError(
            "prolongation must have exactly native and retained-state dimensions; "
            f"got {basis_prolongation.dims!r}"
        )
    if observation_dim not in native_sensitivity.indexes:
        raise ValueError(f"native_sensitivity is missing observation coordinate {observation_dim!r}")
    if state_dim not in basis_prolongation.indexes:
        raise ValueError(f"basis_prolongation is missing retained-state coordinate {state_dim!r}")
    for role, array, dims in (
        ("native_sensitivity", native_sensitivity, (observation_dim, *native_dims)),
        ("basis_prolongation", basis_prolongation, (state_dim, *native_dims)),
    ):
        for dim in dims:
            if dim not in array.indexes:
                raise ValueError(f"{role} is missing labelled index {dim!r}")
            if not array.get_index(dim).is_unique:
                raise ValueError(f"{role} index {dim!r} must contain unique labels")
    if native_sensitivity.sizes[observation_dim] == 0:
        raise ValueError("native_sensitivity must contain at least one observation")
    if basis_prolongation.sizes[state_dim] == 0:
        raise ValueError("basis_prolongation must contain at least one retained state")
    if is_chunked_array(basis_prolongation.data):
        raise ValueError(
