...
class SLSVisualizationFrame:
    ...
    Attributes:
        ...
        current_score: Gaussian benchmark DFS for ``state``.
        best_score: Best Gaussian benchmark DFS found through this iteration.
...
def render_search_gif(
    ...
    *,
    ...
    try:
        ...
        _configure_score_axis(score_axis, iterations, current_scores, best_scores)
        figure.suptitle(title)
        ...
        def update(frame_index: int) -> tuple[LineCollection, ...]:
            ...
            diagnostic_text.set_text(_diagnostic_text(frame))
...
def _configure_score_axis(
    ...
    Args:
        ...
        current_scores: Current Gaussian benchmark DFS values.
        best_scores: Best-so-far Gaussian benchmark DFS values.
    """
    x_min = float(iterations[0])
    x_max = float(iterations[-1])
    x_padding = max((x_max - x_min) * 0.03, 0.5)
    all_scores = np.concatenate((current_scores, best_scores))
    ...
    axis.set_ylabel(_SCORE_NAME)
    axis.set_title("Score history")
    axis.grid(alpha=0.25)
    axis.legend(loc="lower right")


def _diagnostic_text(frame: SLSVisualizationFrame) -> str:
    ...
    return (
        ...
        f"Current {_SCORE_NAME}: {frame.current_score:.4g}\n"
        f"Best {_SCORE_NAME}: {frame.best_score:.4g}"
    )
