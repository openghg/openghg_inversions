...
def addaveragingerror(
    fp_all, sites, species, start_date, end_date, meas_period, inlet=None, instrument=None, stores=None
):
    """
    ...
    """
    if stores is None:
        stores = [None] * len(sites)
    if inlet is None:
        inlet = [None] * len(sites)
    if instrument is None:
        instrument = [None] * len(sites)
