...
- Adds a public mathematical value similar in role to `BasisFunctions`, but representing an affine state-to-quantity operation rather than basis geometry.
- Adds a CO2-specific companion without changing the `Co2PreparedInputs.inv_inputs` model-input schema.
- Adds versioned NetCDF/Zarr round trips, binding validation, dense affine oracles, and public Verification Games parity fixtures.
