...
submit_factor4() {
    ...
    pair="$(submit_job pair-factor4 10-pair-array.sbatch "" --mem 8G --time 06:00:00 --export "${export_value}")"
    oracle="$(submit_job oracle-factor4 11-oracle-array.sbatch "" --mem 8G --time 06:00:00 --export "${export_value}")"
    analysis="$(submit_job analysis-factor4 13-analyze-array.sbatch "${pair}" --mem 8G --time 02:00:00 --export "${export_value}")"
    local_p0="$(submit_job local-p0-factor4 14-local-p0-array.sbatch "${pair}" --time 12:00:00 --export "${export_value}")"
    local_pstar="$(submit_job local-pstar-factor4 15-local-pstar-array.sbatch "${oracle}" --time 12:00:00 --export "${export_value}")"
    conditional="$(submit_job conditional-factor4 16-conditional-array.sbatch "${local_p0}:${local_pstar}:${analysis}" --mem 8G --time 02:00:00 --export "${export_value}")"
...
continue_factor4() {
    ...
    aggregate="$(submit_job aggregate-factor4 18-aggregate.sbatch "${index}" --time 04:00:00 \
        --export "ALL,RUN_ROOT=${RUN_ROOT},REPO_ROOT=${REPO_ROOT},HARNESS_DIR=${HARNESS_DIR},FULL_SHA=${FULL_SHA},PIXI_LOCK_SHA256=${PIXI_LOCK_SHA256},HARNESS_SHA256=${HARNESS_SHA256},PIXI_EXE=${PIXI_EXE},BRANCH_PROFILE=factor4")"
