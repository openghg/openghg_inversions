...
submit_primary() {
    local preflight flow materialize pair oracle nuts analysis local_p0 local_pstar
    ...
    oracle="$(submit_job oracle-primary 11-oracle-array.sbatch "${materialize}" --export "ALL,RUN_ROOT=${RUN_ROOT},REPO_ROOT=${REPO_ROOT},HARNESS_DIR=${HARNESS_DIR},FULL_SHA=${FULL_SHA},PIXI_LOCK_SHA256=${PIXI_LOCK_SHA256},HARNESS_SHA256=${HARNESS_SHA256},PIXI_EXE=${PIXI_EXE},BRANCH_PROFILE=primary")"
    nuts="$(submit_job nuts-primary 12-nuts-array.sbatch "${materialize}" --export "ALL,RUN_ROOT=${RUN_ROOT},REPO_ROOT=${REPO_ROOT},HARNESS_DIR=${HARNESS_DIR},FULL_SHA=${FULL_SHA},PIXI_LOCK_SHA256=${PIXI_LOCK_SHA256},HARNESS_SHA256=${HARNESS_SHA256},PIXI_EXE=${PIXI_EXE},NUTS_PROFILE=primary")"
    analysis="$(submit_job analysis-primary 13-analyze-array.sbatch "${pair}" --export "ALL,RUN_ROOT=${RUN_ROOT},REPO_ROOT=${REPO_ROOT},HARNESS_DIR=${HARNESS_DIR},FULL_SHA=${FULL_SHA},PIXI_LOCK_SHA256=${PIXI_LOCK_SHA256},HARNESS_SHA256=${HARNESS_SHA256},PIXI_EXE=${PIXI_EXE},BRANCH_PROFILE=primary")"
    local_p0="$(submit_job local-p0-primary 14-local-p0-array.sbatch "${pair}" --export "ALL,RUN_ROOT=${RUN_ROOT},REPO_ROOT=${REPO_ROOT},HARNESS_DIR=${HARNESS_DIR},FULL_SHA=${FULL_SHA},PIXI_LOCK_SHA256=${PIXI_LOCK_SHA256},HARNESS_SHA256=${HARNESS_SHA256},PIXI_EXE=${PIXI_EXE},BRANCH_PROFILE=primary")"
    ...
    printf '\nprimary stage submitted: pair=%s oracle=%s nuts=%s analysis=%s local_p0=%s local_pstar=%s\n' \
        "${pair}" "${oracle}" "${nuts}" "${analysis}" "${local_p0}" "${local_pstar}"
}
