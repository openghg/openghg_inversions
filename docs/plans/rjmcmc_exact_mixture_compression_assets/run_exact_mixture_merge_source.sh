...
decision="${decision_directory}/common-source-decision.json"
raw_sha_record="${decision_directory}/common-source-decision.raw.sha256"
complete="${decision_directory}/SOURCE_MERGE_COMPLETE.txt"
case_ids=(
...
for path in "${decision}" "${raw_sha_record}" "${complete}"; do
  if [[ -e "${path}" || -L "${path}" ]]; then
    echo "Refusing to replace source-merge evidence: ${path}" >&2
    exit 2
  fi
done
...
if [[ "${#marker_entries[@]}" -ne 6 ]]; then
  echo "Source marker directory must contain exactly six entries." >&2
  exit 2
fi
for case_id in "${case_ids[@]}"; do
  marker="${marker_directory}/${case_id}.complete"
  expected="complete revision=${MIX_REVISION} stage=source case=${case_id}"
  if [[ ! -f "${marker}" || -L "${marker}" || "$(<"${marker}")" != "${expected}" ]]; then
    echo "Source completion marker is absent or invalid: ${marker}" >&2
    exit 2
  fi
done
...
pixi run --frozen --no-install -e dev \
  ...
  merge-source \
  --report-directory "${source_directory}" \
...
eligible="$(pixi run --frozen --no-install -e dev python -c \
  ...
  "${decision}")"
printf 'Exact-mixture source merge complete for %s eligible=%s raw_sha256=%s\n' \
