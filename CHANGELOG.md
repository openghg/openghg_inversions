...
- Added coordinate-preserving loaders and a weights-first region-constrained
  fixed-outer adapter. Packaged InTEM and binary land/sea fields are normalized
  to the weights grid, outer classes retain one state each, and the requested
  basis count is allocated only across bounded inner classes. The legacy
  weighted fixed-outer output is unchanged. This advances
